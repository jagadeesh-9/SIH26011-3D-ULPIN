"""
SIH26011 Phase 3.7K: Seed Persistent OpenStreetMap-Anchored Canonical Indian Apartment Dataset.

Anchors Surya Heights directly onto verified OpenStreetMap building geometry (Way 356027047, Kondapur, Hyderabad).

Inserts:
1. Synthetic Base Parcel '36A1B2C3D4E5F9' (SY-142/OSM (Synthetic Demo Parcel - OSM Anchor Context), 1400.00 sqm, EPSG:32644)
2. Anchored Building 'APARTMENT-SURYA-OSM' (Surya Heights Residential Apartment — OSM Reference Anchor)
3. 8 3D Vertical Units constructed by extruding the exact verified OSM footprint [EPSG:32644 Area: 378.94 sqm]:
   - B01: Basement Parking Level [Z 536.50 - 540.00] (SB + PARKING)
   - F00: Ground Floor Stilt & Lobby [Z 540.00 - 543.50] (F + COMMON_CIRCULATION)
   - F01: Floor 1 Residential Strata [Z 543.50 - 546.50] (F + RESIDENTIAL)
   - F02: Floor 2 Residential Strata [Z 546.50 - 549.50] (F + RESIDENTIAL)
   - F03: Floor 3 Residential Strata [Z 549.50 - 552.50] (F + RESIDENTIAL)
   - F04: Floor 4 Residential Strata [Z 552.50 - 555.50] (F + RESIDENTIAL)
   - F05: Floor 5 Residential Strata [Z 555.50 - 558.50] (F + RESIDENTIAL)
   - RF01: Rooftop Common Terrace & Solar [Z 558.50 - 561.50] (AR + COMMON_CIRCULATION)
4. Source Evidence records linked to OpenStreetMap Way 356027047 (ODbL 1.0, Attribution: © OpenStreetMap contributors).
5. Initial Verification Audit records in PROPOSED status.

DISCLAIMER:
The 2D building footprint is derived from OpenStreetMap (Way 356027047).
All vertical strata, elevations, floor boundaries, basement volume, and Prototype 3D Unit IDs are
strictly synthetic research demonstration models. Does not represent official Government of India
ULPINs, statutory cadastral boundaries, municipal tax records, or legal land titles.
"""
import os
import sys
import json
import uuid
import psycopg
from dotenv import load_dotenv

sys.path.insert(0, os.path.abspath("."))
from scripts.reconstruct_3d_units import build_polyhedralsurface_wkt_from_footprint

load_dotenv()

PARCEL_ULPIN = "36A1B2C3D4E5F9"
BUILDING_CODE = "APARTMENT-SURYA-OSM"
OSM_WAY_ID = 356027047

def seed_osm_anchored_apartment():
    conn_str = os.getenv("DATABASE_URL", "postgresql://postgres:8919223622@localhost:5432/sih26011_dev")
    if conn_str.startswith("postgresql+psycopg://"):
        conn_str = conn_str.replace("postgresql+psycopg://", "postgresql://")
    
    conn = psycopg.connect(conn_str)
    conn.autocommit = False

    try:
        with conn.cursor() as cur:
            print("--- Seeding OpenStreetMap-Anchored Canonical Indian Apartment (Surya Heights) ---")

            # 1. Handle Idempotency for Parcel
            cur.execute("SELECT id FROM parcels WHERE ulpin_2d = %s;", (PARCEL_ULPIN,))
            p_row = cur.fetchone()
            if p_row:
                parcel_id = p_row[0]
                print(f"Parcel {PARCEL_ULPIN} already exists (ID: {parcel_id}). Cleaning child records for clean idempotent re-seed...")
                cur.execute("DELETE FROM verification_audit WHERE unit_id IN (SELECT id FROM vertical_units WHERE parcel_id = %s);", (parcel_id,))
                cur.execute("DELETE FROM source_evidence WHERE unit_id IN (SELECT id FROM vertical_units WHERE parcel_id = %s);", (parcel_id,))
                cur.execute("DELETE FROM vertical_units WHERE parcel_id = %s;", (parcel_id,))
                cur.execute("DELETE FROM buildings WHERE parcel_id = %s;", (parcel_id,))
            else:
                parcel_id = uuid.uuid4()
                # Parcel footprint: 40m x 35m [X: 219978 to 220018, Y: 1933080 to 1933115] (1400.00 sqm)
                parcel_wkt = "POLYGON((219978.0 1933080.0, 220018.0 1933080.0, 220018.0 1933115.0, 219978.0 1933115.0, 219978.0 1933080.0))"
                cur.execute("""
                    INSERT INTO parcels (
                        id, ulpin_2d, survey_number, district, state, village_code, area_sqm, geom_2d
                    ) VALUES (
                        %s, %s, 'SY-142/OSM (Synthetic Demo Parcel - OSM Anchor Context)', 'Hyderabad', 'Telangana', 'VIL-HYD-KONDAPUR',
                        1400.00, ST_SetSRID(ST_GeomFromText(%s), 32644)
                    );
                """, (parcel_id, PARCEL_ULPIN, parcel_wkt))
                print(f"Created Parcel: {PARCEL_ULPIN} (ID: {parcel_id})")

            # 2. Create Building APARTMENT-SURYA-OSM with exact verified OSM Way 356027047 Footprint (378.94 sqm)
            building_id = uuid.uuid4()
            bldg_footprint_wkt = "POLYGON((219986.82 1933088.62, 219985.71 1933104.81, 220009.01 1933106.37, 220010.12 1933090.19, 219986.82 1933088.62))"
            cur.execute("""
                INSERT INTO buildings (
                    id, parcel_id, building_code, building_name, total_floors_above, total_floors_below, footprint_2d
                ) VALUES (
                    %s, %s, %s, 'Surya Heights Residential Apartment — OSM Reference Anchor', 6, 1,
                    ST_SetSRID(ST_GeomFromText(%s), 32644)
                );
            """, (building_id, parcel_id, BUILDING_CODE, bldg_footprint_wkt))
            print(f"Created Building: {BUILDING_CODE} (ID: {building_id})")

            # 3. Exact OSM Footprint Coordinates in EPSG:32644
            osm_footprint = [
                [219986.82, 1933088.62],
                [219985.71, 1933104.81],
                [220009.01, 1933106.37],
                [220010.12, 1933090.19],
                [219986.82, 1933088.62]
            ]

            # Common OSM Provenance Metadata JSON
            osm_meta_base = {
                "source": "OpenStreetMap",
                "element_type": "way",
                "element_id": OSM_WAY_ID,
                "building_tag": "yes",
                "license": "ODbL 1.0",
                "attribution": "© OpenStreetMap contributors",
                "source_role": "REFERENCE_BUILDING_FOOTPRINT",
                "vertical_geometry_role": "NOT_SOURCE_DERIVED",
                "is_synthetic": True,
                "prototype_only": True,
            }

            unit_definitions = [
                {
                    "tier_code": "SB",
                    "floor_code": "B01",
                    "unit_type": "PARKING",
                    "unit_label": "Basement Parking Level (OSM Anchor Demo)",
                    "z_min": 536.50,
                    "z_max": 540.00,
                    "footprint": osm_footprint,
                    "source_type": "MANUAL_DIGITIZED",
                    "dataset_name": f"OpenStreetMap Way {OSM_WAY_ID} (Footprint Anchor)",
                    "file_uri": f"https://www.openstreetmap.org/way/{OSM_WAY_ID}",
                    "acc_h": 0.10,
                    "acc_v": 0.05,
                    "meta": {
                        **osm_meta_base,
                        "description": "Subterranean vehicle parking and mechanical storage.",
                        "safety_statement": "Basement geometry is synthetic research demonstration geometry and is not established by OpenStreetMap or LiDAR."
                    }
                },
                {
                    "tier_code": "F",
                    "floor_code": "F00",
                    "unit_type": "COMMON_CIRCULATION",
                    "unit_label": "Ground Floor Stilt & Entrance Lobby",
                    "z_min": 540.00,
                    "z_max": 543.50,
                    "footprint": osm_footprint,
                    "source_type": "MANUAL_DIGITIZED",
                    "dataset_name": f"OpenStreetMap Way {OSM_WAY_ID} (Footprint Anchor)",
                    "file_uri": f"https://www.openstreetmap.org/way/{OSM_WAY_ID}",
                    "acc_h": 0.10,
                    "acc_v": 0.05,
                    "meta": {
                        **osm_meta_base,
                        "description": "Ground floor stilt parking bay, entrance lobby and security control.",
                        "safety_statement": "Vertical strata are synthetic research demonstration geometry generated from the OSM reference footprint."
                    }
                },
                {
                    "tier_code": "F",
                    "floor_code": "F01",
                    "unit_type": "RESIDENTIAL",
                    "unit_label": "Floor 1 Residential Strata (Flats 101-104)",
                    "z_min": 543.50,
                    "z_max": 546.50,
                    "footprint": osm_footprint,
                    "source_type": "MANUAL_DIGITIZED",
                    "dataset_name": f"OpenStreetMap Way {OSM_WAY_ID} (Footprint Anchor)",
                    "file_uri": f"https://www.openstreetmap.org/way/{OSM_WAY_ID}",
                    "acc_h": 0.10,
                    "acc_v": 0.05,
                    "meta": {
                        **osm_meta_base,
                        "description": "First floor multi-dwelling residential strata.",
                        "safety_statement": "Vertical strata are synthetic research demonstration geometry generated from the OSM reference footprint."
                    }
                },
                {
                    "tier_code": "F",
                    "floor_code": "F02",
                    "unit_type": "RESIDENTIAL",
                    "unit_label": "Floor 2 Residential Strata (Flats 201-204)",
                    "z_min": 546.50,
                    "z_max": 549.50,
                    "footprint": osm_footprint,
                    "source_type": "MANUAL_DIGITIZED",
                    "dataset_name": f"OpenStreetMap Way {OSM_WAY_ID} (Footprint Anchor)",
                    "file_uri": f"https://www.openstreetmap.org/way/{OSM_WAY_ID}",
                    "acc_h": 0.10,
                    "acc_v": 0.05,
                    "meta": {
                        **osm_meta_base,
                        "description": "Second floor multi-dwelling residential strata.",
                        "safety_statement": "Vertical strata are synthetic research demonstration geometry generated from the OSM reference footprint."
                    }
                },
                {
                    "tier_code": "F",
                    "floor_code": "F03",
                    "unit_type": "RESIDENTIAL",
                    "unit_label": "Floor 3 Residential Strata (Flats 301-304)",
                    "z_min": 549.50,
                    "z_max": 552.50,
                    "footprint": osm_footprint,
                    "source_type": "MANUAL_DIGITIZED",
                    "dataset_name": f"OpenStreetMap Way {OSM_WAY_ID} (Footprint Anchor)",
                    "file_uri": f"https://www.openstreetmap.org/way/{OSM_WAY_ID}",
                    "acc_h": 0.10,
                    "acc_v": 0.05,
                    "meta": {
                        **osm_meta_base,
                        "description": "Third floor multi-dwelling residential strata.",
                        "safety_statement": "Vertical strata are synthetic research demonstration geometry generated from the OSM reference footprint."
                    }
                },
                {
                    "tier_code": "F",
                    "floor_code": "F04",
                    "unit_type": "RESIDENTIAL",
                    "unit_label": "Floor 4 Residential Strata (Flats 401-404)",
                    "z_min": 552.50,
                    "z_max": 555.50,
                    "footprint": osm_footprint,
                    "source_type": "MANUAL_DIGITIZED",
                    "dataset_name": f"OpenStreetMap Way {OSM_WAY_ID} (Footprint Anchor)",
                    "file_uri": f"https://www.openstreetmap.org/way/{OSM_WAY_ID}",
                    "acc_h": 0.10,
                    "acc_v": 0.05,
                    "meta": {
                        **osm_meta_base,
                        "description": "Fourth floor multi-dwelling residential strata.",
                        "safety_statement": "Vertical strata are synthetic research demonstration geometry generated from the OSM reference footprint."
                    }
                },
                {
                    "tier_code": "F",
                    "floor_code": "F05",
                    "unit_type": "RESIDENTIAL",
                    "unit_label": "Floor 5 Residential Strata (Flats 501-504)",
                    "z_min": 555.50,
                    "z_max": 558.50,
                    "footprint": osm_footprint,
                    "source_type": "MANUAL_DIGITIZED",
                    "dataset_name": f"OpenStreetMap Way {OSM_WAY_ID} (Footprint Anchor)",
                    "file_uri": f"https://www.openstreetmap.org/way/{OSM_WAY_ID}",
                    "acc_h": 0.10,
                    "acc_v": 0.05,
                    "meta": {
                        **osm_meta_base,
                        "description": "Fifth floor multi-dwelling residential strata.",
                        "safety_statement": "Vertical strata are synthetic research demonstration geometry generated from the OSM reference footprint."
                    }
                },
                {
                    "tier_code": "AR",
                    "floor_code": "RF01",
                    "unit_type": "COMMON_CIRCULATION",
                    "unit_label": "Rooftop Common Terrace & Solar Grid",
                    "z_min": 558.50,
                    "z_max": 561.50,
                    "footprint": osm_footprint,
                    "source_type": "MANUAL_DIGITIZED",
                    "dataset_name": f"OpenStreetMap Way {OSM_WAY_ID} (Footprint Anchor)",
                    "file_uri": f"https://www.openstreetmap.org/way/{OSM_WAY_ID}",
                    "acc_h": 0.10,
                    "acc_v": 0.05,
                    "meta": {
                        **osm_meta_base,
                        "description": "Rooftop common terrace, solar panel grid, and lift machine room.",
                        "safety_statement": "Vertical strata are synthetic research demonstration geometry generated from the OSM reference footprint."
                    }
                }
            ]

            for u_def in unit_definitions:
                # Concurrency-safe sequence allocation
                cur.execute("SELECT nextval('vertical_unit_seq');")
                seq_val = cur.fetchone()[0]
                
                tier = u_def["tier_code"]
                proto_ulpin = f"{PARCEL_ULPIN}-3D-{tier}-{seq_val:04d}"
                unit_id = uuid.uuid4()

                # Build watertight 3D PolyhedralSurface WKT from exact OSM footprint
                wkt, _ = build_polyhedralsurface_wkt_from_footprint(
                    u_def["footprint"],
                    u_def["z_min"],
                    u_def["z_max"]
                )

                unit_seq = (seq_val - 1) % 9999 + 1
                # Insert vertical unit strictly as PROPOSED
                cur.execute("""
                    INSERT INTO vertical_units (
                        id, parcel_id, building_id, prototype_ulpin_3d, tier_code, floor_code,
                        unit_sequence, unit_label, unit_type, z_min, z_max, geom_3d, status
                    ) VALUES (
                        %s, %s, %s, %s, %s, %s,
                        %s, %s, %s, %s, %s, ST_SetSRID(ST_GeomFromText(%s), 32644), 'PROPOSED'
                    );
                """, (
                    unit_id, parcel_id, building_id, proto_ulpin, tier, u_def["floor_code"],
                    unit_seq, u_def["unit_label"], u_def["unit_type"], u_def["z_min"], u_def["z_max"],
                    wkt
                ))

                # Attach source evidence linking to OSM Way 356027047
                evidence_id = uuid.uuid4()
                cur.execute("""
                    INSERT INTO source_evidence (
                        id, unit_id, source_type, dataset_name, file_uri,
                        accuracy_horizontal_m, accuracy_vertical_m, metadata_json
                    ) VALUES (
                        %s, %s, %s, %s, %s,
                        %s, %s, %s
                    );
                """, (
                    evidence_id, unit_id, u_def["source_type"], u_def["dataset_name"], u_def["file_uri"],
                    u_def["acc_h"], u_def["acc_v"], json.dumps(u_def["meta"])
                ))

                # Attach initial AUTO_INGESTION verification audit
                audit_id = uuid.uuid4()
                cur.execute("""
                    INSERT INTO verification_audit (
                        id, unit_id, action, previous_status, new_status,
                        reviewer_name, reviewer_role, review_notes, integrity_hash
                    ) VALUES (
                        %s, %s, 'AUTO_INGESTION', 'PROPOSED', 'PROPOSED',
                        'OSM Reference Ingestion Engine', 'SYSTEM_VALIDATOR',
                        %s, %s
                    );
                """, (
                    audit_id, unit_id,
                    f"Automated ingestion of OSM-anchored {u_def['floor_code']} strata. Initialized as PROPOSED.",
                    f"osm_hash_auto_{proto_ulpin}"
                ))

                print(f"  Inserted OSM Unit {u_def['floor_code']} (Seq {seq_val}): {proto_ulpin} | Status: PROPOSED")

            conn.commit()
            print(f"--- Successfully Persisted OpenStreetMap-Anchored Building ({BUILDING_CODE}) ---")

    except Exception as e:
        conn.rollback()
        print(f"Error seeding OSM-Anchored Apartment: {e}")
        raise e
    finally:
        conn.close()

if __name__ == "__main__":
    seed_osm_anchored_apartment()
